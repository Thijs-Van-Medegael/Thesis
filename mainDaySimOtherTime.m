...
ToUShifted = circshift(totalToU+baseload,-beginPlot);
...
totalIteration = totalDumb+baseload;
...
if sum(totalFinal) - sum(totalDumb+baseload) > 1^(-10)
...
while max(totalIteration2+baseload) > cap
    demandIteration2 = zeros(EVnumber,size(t1,2));
    congestionPrice2 = arrayfun(@(x)w*max(x-cap,0)/cap,totalIteration2 + baseload); 
...
totalFinal2 = totalIteration2 + baseload;
